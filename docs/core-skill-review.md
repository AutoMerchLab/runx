...
| issue-to-pr | workflow | public/canonical | javascript, tool:provider.mutate, tool:provider.read; 1 agent act -> declared artifact | harness passed; 0 blocking finding(s); 2 operation proof(s); 1 agent-contract proof(s); 1 operator journey(s); provider readback unproven | improve | The host now edits and tests normally, calls finalize once, and returns a typed host result; optional PR publication is one approved mutation/readback with no mandatory feed, outbox, or duplicated lifecycle. | Capture one explicitly approved harmless draft-PR push with stable provider readback before treating provider completion as proven. |
...
| send-as | workflow | public/canonical | javascript, tool:provider.mutate, tool:provider.read; 1 agent act -> declared artifact | complete archetype bar | keep | The default now plans, performs one exact approved provider-neutral send, and verifies readback; plan, apply, and verify consume typed prior artifacts without repeating work. | none |
