...
opens an exactly approved pull request and reads it back.
...
7. When publication is requested, approve the exact `pullrequest.open` effect,
   create the PR once, and independently read the resulting PR back.
...
Reads and local analysis require no approval. The only mandatory approval is
the consequential PR creation boundary. Its gate binds provider, repository,
operation, payload digest, required scope, principal, and plan digest.

Runx persists provider attempt and readback state under the project `.runx`
store. Resume retains the approved plan and generated idempotency binding. If a
...
If Runx pauses at host work or approval, continue the same immutable run:

```bash
runx resume <run-id> -
```

The stdin document carries the requested agent answer or the exact human
approval. Normal output stays compact; use `--diagnostics` only when graph
internals are actually needed.
...
finalize claim, chain reuse without repeated work, a single exact publication
approval, one PR mutation, independent PR readback, and the absence of implicit
feed or notification work.
