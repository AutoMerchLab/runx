...
#[test]
fn payment_journey_resumes_approval_then_agent_once_and_cannot_replay_completion() -> TestResult {
    let root = crate::support::temp_root("runx-payment-resume-journey");
    let repo_root = crate::support::repo_root()?;
    let receipt_dir = root.join(".runx/receipts");
    let inputs = root.join("payment-inputs.json");
    fs::create_dir_all(&root)?;
    fs::write(&inputs, payment_inputs().to_string())?;

    let pause = command(&root)
        .arg("skill")
        .arg(repo_root.join("skills/spend"))
        .arg("--inputs")
        .arg("payment-inputs.json")
        .arg("--receipt-dir")
        .arg(&receipt_dir)
    ...
    let approval_id = pause["requests"]
        ...
        .and_then(|requests| requests.first())
        .and_then(|request| request["id"].as_str())
        .ok_or("payment pause omitted its approval request")?;
    let approval = serde_json::json!({
        "approvals": {
            (approval_id): {
                "approved": true,
                "reason": "Settle the exact quoted amount through the selected MPP rail."
            }
        }
    })
    .to_string();
    let mut approve = command(&root);
    approve
        .arg("resume")
        .arg(run_id)
        .arg("-")
    ...
    let agent_pause = output_with_stdin(&mut approve, &approval)?;
    let agent_pause = assert_json(&agent_pause, 2)?;
    assert_eq!(agent_pause["status"], "needs_agent", "{agent_pause}");
    assert_eq!(agent_pause["run_id"], run_id);
    let agent_request = agent_pause["requests"]
        .as_array()
        .and_then(|requests| requests.first())
        .and_then(|request| request["id"].as_str())
        .ok_or("payment resume omitted its rail request")?;
    let rail_answer = serde_json::json!({
        "answers": {
            (agent_request): {
                "rail_result": {
                    "status": "fulfilled",
                ...
                "rail_proof": {
                    "proof_ref": "receipt-proof:mpp:operator-journey-001",
                    "idempotency_key": "payment:operator-journey-001"
                },
                "credential_envelope": {
                    "form": "paid_tool_credential",
                    "credential_ref": "credential:mpp:operator-journey-001"
                },
    ...
    fulfill
        ...
        .arg(&receipt_dir)
        .args(["--json", "--diagnostics"]);
    let completed = output_with_stdin(&mut fulfill, &rail_answer)?;
    let completed = assert_json(&completed, 0)?;
    assert_eq!(completed["status"], "sealed", "{completed}");
    assert_eq!(completed["outcome"], "completed");
    assert_eq!(completed["run_id"], run_id);
    let rendered = completed.to_string();
    assert!(rendered.contains("fulfilled"));
    assert!(rendered.contains("receipt-proof:mpp:operator-journey-001"));
    let receipt_id = json_string(&completed, "receipt_id")?;
    assert_receipt_verifies(&root, &receipt_dir, receipt_id)?;
    ...
    duplicate
        ...
        .arg(&receipt_dir)
        .arg("--json");
    let duplicate = output_with_stdin(&mut duplicate, &rail_answer)?;
    let duplicate = assert_json(&duplicate, 1)?;
    assert_eq!(duplicate["status"], "failure");
    assert!(
        duplicate["error"]["message"]
            .as_str()
            .is_some_and(|message| message.contains("pending") || message.contains("resume"))
    );
    Ok(())
}

fn payment_inputs() -> Value {
    serde_json::json!({
        "payment_signal": {
            "signal_type": "effect_required",
            "challenge_id": "ch_mpp_operator_journey_001",
            "amount_minor": 125,
            "currency": "USD",
            "rail": "mpp",
        ...
        "parent_payment_authority": {
            "term_id": "authority-term:payment:operator-journey-001",
        ...
        "realm": "test",
        "idempotency_seed": "operator-journey-001"
    })
}
