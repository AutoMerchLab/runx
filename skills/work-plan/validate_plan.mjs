...
try {
  normalized = validateDraft(draft);
} catch (error) {
  findings.push({
...
function validateDraft(value) {
  ...
  const sourceChangeSetPreserved = suppliedChangeSet ? isDeepStrictEqual(changeSet, suppliedChangeSet) : false;
  ...
  const plan = validateWorkspacePlan(value.workspace_change_plan, changeSet, objectiveSummary);
  const requiredSkills = validateRequiredSkills(value.required_skills);
  const steps = validateOrchestration(value.orchestration_steps, requiredSkills);
  ...
  if (!isDeepStrictEqual(openQuestions, plan.open_questions)) {
...
function validateWorkspacePlan(value, changeSet, objectiveSummary) {
  ...
  if (!isDeepStrictEqual(invariants, strings(changeSet.shared_invariants, "change_set.shared_invariants"))) {
  ...
  if (!isDeepStrictEqual(criteria, strings(changeSet.success_criteria, "change_set.success_criteria"))) {
...
function validateRequiredSkills(value) {
...
function validateOrchestration(value, requiredSkills) {
...
function discoverCatalog(workspaceRoot) {
  const skillsRoot = path.join(workspaceRoot, "skills");
  if (!fs.existsSync(skillsRoot)) return [];
  return fs.readdirSync(skillsRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && !entry.name.startsWith("."))
    .filter((entry) => fs.existsSync(path.join(skillsRoot, entry.name, "X.yaml")))
    .map((entry) => ({ name: entry.name, path: `skills/${entry.name}/X.yaml` }))
